...
import kotlinx.android.synthetic.main.activity_district_panchayat_summary.*
import kotlinx.android.synthetic.main.activity_district_panchayat_summary.fincyear
import kotlinx.android.synthetic.main.activity_district_panchayat_summary.mblock
import kotlinx.android.synthetic.main.activity_district_scheme_wise_beneficiary_list.*
...
import kotlinx.android.synthetic.main.activity_scheme_wise_beneficiary.recyclerView1
import kotlinx.android.synthetic.main.activity_scheme_wise_beneficiary.searchview
import kotlinx.android.synthetic.main.activity_scheme_wise_beneficiary.total_beneficiary_count
...
class DistrictSchemeWiseBeneficiaryList : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_district_scheme_wise_beneficiary_list)
...
//        getPanchayatList()
    ...
    private fun getBeneficiaryList(yearid: String, schemeId: String, panjayatId: String, blockid: String) {
        ...
        beneficiaryRequest.blockid = blockid
    ...
    private fun setRecyclerAdapter() {
        ...
        recyclerView.bind(rResponseResults, R.layout.stagewise_recycler_view_item) { item ->
            ...
            llHeader.setOnClickListener {
                if (!item.stageid.isNullOrEmpty()) {
                    val intent =
                        Intent(this@DistrictSchemeWiseBeneficiaryList, StageWiseBeneficiaryList::class.java)
                    intent.putExtra("schemeid", schemeId)
                    intent.putExtra("panchayatId", mdPanchayat.selectedItems[0].key)
                    intent.putExtra("panchayatname", mdPanchayat.selectedItems[0].name)
        ...
        }.layoutManager(LinearLayoutManager(this@DistrictSchemeWiseBeneficiaryList))
        ...
        mScheme!!.setItems(arrayList1, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
                    ...
                    if (finyear.selectedItems.size > 0 && schemeId != null && mdPanchayat.selectedItems.size > 0 && mblock.selectedItems.size > 0) {
                        getBeneficiaryList(
                            finyear.selectedItems[0].key,
                            schemeId!!,
                            mdPanchayat.selectedItems[0].key,
                            mblock.selectedItems[0].key
                        )
                    }
    ...
    private fun getPanchayatList(blockid: String) {
//        val rResponse = realm!!.where<OTPResponseResults>().findFirst()
//        if (rResponse != null) {
            val blockId = blockid
            APIPresenter(this, object : APIView() {
            ...
            }).getDistrictPanjayatList(blockId.toString())
...
//        }
    ...
    private fun setPanchayatItems() {
        ...
        mdPanchayat!!.setItems(panchayatArray, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
                    Log.i("val_driver", i.toString() + " : " + items[i].key + " : " + items[i].name)
                    Utils.showProgress(this)
    ...
    private fun setBlockItems() {
        val arrayList = ArrayList<KeyPairModel>()
        val query1 = realm!!.where<BlockResponseResults>().findAll()
        for (k in 0 until query1.size) {
            if (k == 0) {
                arrayList!!.add(
                    KeyPairModel(
        ...
        mblock!!.setItems(arrayList1, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
                    ...
                    if (finyear.selectedItems.size > 0 && schemeId != null && mdPanchayat.selectedItems.size > 0 && mblock.selectedItems.size > 0) {
                        ...
                        getPanchayatList(mblock.selectedItems[0].key)
                    }
        ...
        if (mblock.selectedItems.size > 0) {
