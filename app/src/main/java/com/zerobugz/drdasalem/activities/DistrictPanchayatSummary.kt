...
import kotlinx.android.synthetic.main.activity_panchayat_summary.fincyear
...
class DistrictPanchayatSummary : AppCompatActivity() {
    private var realm: Realm? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_district_panchayat_summary)
...
//        getSchemaList()
    ...
    private fun getBeneficiaryList(schemeId: String, blockid: String, yearid: String) {
        swipeRefresh.isRefreshing = true
        ...
        recyclerView.bind(listResponse, R.layout.summary_recycler_view_item) { item ->
            ...
            llHeader.setOnClickListener {
                ...
                if(item.stagelist!!.size>0){
                    val dialog = BottomSheetDialog(this@DistrictPanchayatSummary)
        ...
        mScheme!!.setItems(arrayList1, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
                    ...
                    if (fincyear.selectedItems.size > 0 && mScheme.selectedItems.size > 0 && mblock.selectedItems.size > 0) {
