...
import kotlinx.android.synthetic.main.activity_block_summary.*
...
class BlockSummary : AppCompatActivity() {
    private var realm: Realm? = null


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_block_summary)
        realm = try {
...
//        getSchemaList()
    ...
    private fun setSearchRecyclerAdapter(newText: String) {
        ...
        val rResponseResults = realm!!.where<BlockWiseListResponse>()
            .contains("blockname", newText)
            .findAll()
    ...
    private fun getBeneficiaryList(yearid: String, schemeId: String) {
        ...
        }).getBlockWiseCountList(beneficiaryRequest)
    }

    @SuppressLint("SetTextI18n")
    private fun setRecyclerAdapter() {

        val blockWiseListResponse = realm!!.where<BlockWiseListResponse>().findAll()


        recyclerView.bind(blockWiseListResponse, R.layout.summary_recycler_view_item) { item ->
            ...
            llHeader.setOnClickListener {
                ...
                if (item.stagelist!!.size > 0) {
                    val dialog = BottomSheetDialog(this@BlockSummary)
                    ...
                    val recyclerView =
                        dialog.findViewById<RecyclerView>(R.id.bottomSheetRecyclerView)
                    ...
                    nameToolbar!!.text = "" + item.blockname
        ...
        mScheme!!.setItems(arrayList1, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
                    ...
                    if (finbyear.selectedItems.size > 0 && mScheme.selectedItems.size > 0) {
                        getBeneficiaryList(
                            finbyear.selectedItems[0].key,
    ...
    private fun setYearItems() {
        ...
        finbyear!!.setItems(arrayList1, -1) { items: List<KeyPairBoolData> ->
            for (i in items.indices) {
                if (items[i].isSelected) {
