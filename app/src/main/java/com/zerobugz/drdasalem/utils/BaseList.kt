...
open class FastListAdapter<T>(
    ...
    private inner class BindMap(
        ...
        val predicate: (item: T, idx: Int) -> Boolean
    ) {
        constructor(
            lf: LayoutFactory,
            type: Int = 0,
            bind: View.(item: T) -> Unit,
            predicate: (item: T, idx: Int) -> Boolean
        ) : this(0, type, bind, predicate) {
            layoutFactory = lf
        }
    ...
    fun layoutManagerReversed(manager: LinearLayoutManager): FastListAdapter<T> {
        vpList?.let { throw UnsupportedOperationException("layoumanager not needed for ViewPager2") }
        list!!.layoutManager = manager
